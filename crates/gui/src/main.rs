...
pub(crate) fn dispatch_cmd(ui: &AppWindow, cmd: &str) {
...
pub(crate) struct State {
    ...
    pub(crate) ai_worker_tx: mpsc::Sender<AiWorkerMessage>,
    pub(crate) ai_cancel_token: AiCancelToken,
...
pub(crate) fn open_in_editor(ui: &AppWindow, b: &Backend, note_id: &str) {
...
fn setup_app(vault: PathBuf) -> Result<AppCtx, Box<dyn std::error::Error>> {
    ...
    }
    {
        ...
        ui.on_ai_draft_agenda(move || {
            ...
            ai_controller::enqueue_draft_agenda(&ui, &mut s);
            refresh(&ui, &s);
        });
    }
    {
        ...
        ui.on_ai_review_note(move || {
            ...
            ai_controller::enqueue_note_review(&ui, &mut s);
            refresh(&ui, &s);
        });
    }
    {
        ...
        ui.on_ai_refresh_embeddings(move || {
            ...
            ai_controller::refresh_embeddings(&ui, &mut s);
        });
    }
    {
        let ui_w = ui.as_weak();
        ...
        ui.on_ai_semantic_search(move |query: SharedString| {
            ...
            ai_controller::run_semantic_search(&ui, &mut s, &query);
            refresh(&ui, &s);
        });
    }
    {
        ...
        ui.on_ai_open_semantic_result(move |note_id: SharedString| {
            ...
            ai_controller::open_semantic_result(&ui, &mut s, &note_id);
            refresh(&ui, &s);
        });
    }
    {
    ...
    // Command palette: populate results on query change; dispatch on activate.
    {
        ...
        ui.on_palette_search(move |q| {
            let ui = ui_w.unwrap();
            let items = palette::results(&state.borrow().backend, &q);
    ...
    }
    {
        ...
        ui.on_palette_activate(move |id| {
            palette::activate(&ui_w.unwrap(), &id);
        });
    }
        ...
        timer.start(
            ...
            move || {
                ...
                while let Ok(message) = ai_worker_rx.try_recv() {
                    let mut s = state.borrow_mut();
                    ai_controller::apply_worker_message(&ui, &mut s, message);
                    applied = true;
                }
