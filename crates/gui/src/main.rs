...
struct PreviewEmbeddingRuntime;
...
fn local_runtime_settings_from_state(state: &State) -> LocalRuntimeSettings {
    let runtime_bin = state.app.ai.settings.runtime_bin.clone();
    let model_root = state.app.ai.settings.model_root.clone();
    let timeout_seconds = state.app.ai.settings.timeout_seconds;
...
fn use_preview_ai_runtime() -> bool {
...
fn local_model_specs(model_root: &Path) -> BTreeMap<String, LocalModelSpec> {
    [
        (
...
fn require_free_memory(min_percent: u8) -> Result<(), String> {
