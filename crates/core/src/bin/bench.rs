...
const WORKSTREAMS: &[&str] = &[
    "Platform", "Platform/Auth", "Platform/Billing", "Growth", "Growth/SEO",
    "Hiring", "Hiring/Eng", "OKRs", "OKRs/Q3", "Infra", "Infra/Migration", "Support",
];
const PEOPLE: &[&str] = &[
...
const TAGS: &[&str] = &[
    "urgent", "blocked", "idea", "decision", "risk", "followup",
    "1on1", "roadmap", "bug", "research/ml", "research/ux", "budget",
];
const KINDS: &[&str] = &["do", "followup", "delegated", "todelegate", "someday", "reading"];
...
fn gen_note(rng: &mut Lcg, i: usize) -> String {
    ...
    s.push_str(&format!("Some context paragraph about #{} and #{}. ", rng.pick(TAGS), rng.pick(TAGS)));
    ...
    for _ in 0..rng.range(2, 6) {
        ...
        let pstr = if prio.is_empty() { String::new() } else { format!(" [#{prio}]") };
...
fn time<T>(label: &str, n: usize, f: impl FnOnce() -> T) -> T {
    ...
    let per = if n > 0 { format!("  ({:.3} ms / 1k notes)", ms / n as f64 * 1000.0) } else { String::new() };
...
fn main() -> anyhow::Result<()> {
    ...
    let mut b = time("open() — schema + full index", n, || Backend::open(vault.clone()).unwrap());

    // --- warm full reindex (the file-watch / ⟳ path) ----------------------
    time("reindex_all() — full rebuild", n, || b.reindex_all().unwrap());

    println!("\n  --- queries that refresh() runs every interaction ---");
    let f = Filter::default();
    let cnt = time("query_notes(default)", n, || b.query_notes(&f).unwrap().len());
    println!("    -> {cnt} notes");
    time("query_notes(search='roadmap')", n, || {
        let sf = Filter { search: "roadmap".into(), ..Default::default() };
        b.query_notes(&sf).unwrap().len()
    });
    let tc = time("query_todos(default)", n, || b.query_todos(&f).unwrap().len());
    println!("    -> {tc} todos");
    time("query_todos(status=open)", n, || {
        let sf = Filter { status: "open".into(), ..Default::default() };
        b.query_todos(&sf).unwrap().len()
    });
    time("board(project)", n, || b.board("project", &f).unwrap().len());
