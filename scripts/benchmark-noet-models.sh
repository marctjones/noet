...
OUT_DIR="${OUT_DIR:-$PWD/tmp/noet-model-bench}"
# Keep defaults conservative so local benchmark runs do not evict other apps.
...
Defaults are memory-conservative:
  MAX_SEQ_LEN=1024 MAX_SEQS=1 PREFIX_CACHE_N=0 MIN_FREE_PERCENT=25
EOF
}
...
write_prompt() {
  ...
  case "$kind" in
    labels)
      cat >"$path" <<'EOF'
You are helping Noet organize a local note. Read the note and respond with exactly three short labels, one per line, no bullets, no explanation.
...
EOF
    ...
    long_context)
      {
        printf '%s\n' "You are helping Noet review a long local note archive. Summarize the main blockers in exactly five short bullet points."
        printf '%s\n\n' "Archive excerpt:"
        for i in $(seq 1 8); do
          cat <<EOF
### Note $i
...
model_7b_dir="$(snapshot_dir "$HF_CACHE_ROOT/models--bartowski--Mistral-7B-Instruct-v0.3-GGUF")"
model_8b_dir="$(snapshot_dir "$HF_CACHE_ROOT/models--bartowski--Ministral-8B-Instruct-2410-GGUF")"
model_nemo_dir="$(snapshot_dir "$HF_CACHE_ROOT/models--bartowski--Mistral-Nemo-Instruct-2407-GGUF")"

for dir_var in model_7b_dir model_8b_dir model_nemo_dir; do
  if [[ -z "${!dir_var}" ]]; then
...
if [[ "$RUN_ALL" == "0" ]]; then
  case "$SINGLE_MODEL" in
    mistral7b)
      run_case "mistral7b" "$model_7b_dir" "Mistral-7B-Instruct-v0.3-Q4_K_M.gguf" "$SINGLE_PROMPT" >>"$report_file"
      ;;
    ministral8b)
      run_case "ministral8b" "$model_8b_dir" "Ministral-8B-Instruct-2410-Q4_K_M.gguf" "$SINGLE_PROMPT" >>"$report_file"
      ;;
    mistralnemo)
      run_case "mistralnemo" "$model_nemo_dir" "Mistral-Nemo-Instruct-2407-Q4_K_M.gguf" "$SINGLE_PROMPT" >>"$report_file"
      ;;
    *)
  ...
  run_case "mistral7b" "$model_7b_dir" "Mistral-7B-Instruct-v0.3-Q4_K_M.gguf" "labels" >>"$report_file"
  run_case "mistral7b" "$model_7b_dir" "Mistral-7B-Instruct-v0.3-Q4_K_M.gguf" "tasks" >>"$report_file"
  run_case "mistral7b" "$model_7b_dir" "Mistral-7B-Instruct-v0.3-Q4_K_M.gguf" "patch" >>"$report_file"
  run_case "mistral7b" "$model_7b_dir" "Mistral-7B-Instruct-v0.3-Q4_K_M.gguf" "long_context" >>"$report_file"

  run_case "ministral8b" "$model_8b_dir" "Ministral-8B-Instruct-2410-Q4_K_M.gguf" "labels" >>"$report_file"
  run_case "ministral8b" "$model_8b_dir" "Ministral-8B-Instruct-2410-Q4_K_M.gguf" "tasks" >>"$report_file"
  run_case "ministral8b" "$model_8b_dir" "Ministral-8B-Instruct-2410-Q4_K_M.gguf" "patch" >>"$report_file"
  run_case "ministral8b" "$model_8b_dir" "Ministral-8B-Instruct-2410-Q4_K_M.gguf" "long_context" >>"$report_file"

  run_case "mistralnemo" "$model_nemo_dir" "Mistral-Nemo-Instruct-2407-Q4_K_M.gguf" "labels" >>"$report_file"
  run_case "mistralnemo" "$model_nemo_dir" "Mistral-Nemo-Instruct-2407-Q4_K_M.gguf" "tasks" >>"$report_file"
  run_case "mistralnemo" "$model_nemo_dir" "Mistral-Nemo-Instruct-2407-Q4_K_M.gguf" "patch" >>"$report_file"
  run_case "mistralnemo" "$model_nemo_dir" "Mistral-Nemo-Instruct-2407-Q4_K_M.gguf" "long_context" >>"$report_file"
fi
